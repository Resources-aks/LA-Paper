...
class DToA {
    ...
    static String JS_dtobasestr(int base, double d)
    {
        ...
        if (lfloor == dfloor) {
            // int part fits long
            intDigit = Long.toString((negative) ? -lfloor : lfloor, base);
        } else {
...
//                JS_ASSERT(digit < (uint32)base);
    ...
    static double computeDouble(double d, int i)
    {
        long dBit = Double.doubleToLongBits(d);
                    ...
                    boolean[] sign, StringBuilder buf)
    {
        ...
        */
        ...
        if ((word0(d) & Sign_bit) != 0) {
            ...
            d = computeDouble(d, word0(d) & ~Sign_bit);
        }
        else
            sign[0] = false;
...
//          no_digits:
        ...
        if ((i = (word0(d) >>> Exp_shift1 & (Exp_mask>>Exp_shift1))) != 0) {
            d2 = computeDouble(d, (word0(d) & Frac_mask1) | Exp_11);
...
//            word0(d2) -= 31*Exp_msk1; /* adjust exponent */
            d2 = computeDouble(x, word0(x) - 31*Exp_msk1);
            i -= (Bias + (P-1) - 1) + 1;
            denorm = true;
        }
...
//            word0(eps) -= (P-1)*Exp_msk1;
            eps = ieps*d + 7.0;
            eps = computeDouble(eps, word0(eps) - (P-1)*Exp_msk1);
            if (ilim == 0) {
                S = mhi = null;
                d -= 5.0;
                if (d > eps) {
